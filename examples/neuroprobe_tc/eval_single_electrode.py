...
# Use the same localization directory as in the quickstart example
...
for eval_name in eval_names:
    ...
    for electrode_idx, electrode_label in enumerate(all_electrode_labels):
        ...
        # Electrode label should match the channel name used in the coordinate CSV
        coords_sub_key = subject_identifier_to_coords_key(subject.subject_identifier)

        this_coord = coords_df[
            (coords_df['Subject'] == coords_sub_key) &
            (coords_df['Electrode'] == electrode_label)
        ].iloc[0]

        # (X, Y, Z) coordinate for this single electrode (shape: 1 x 3)
        electrode_xyz = np.array(
            [[this_coord['X'], this_coord['Y'], this_coord['Z']]],
            dtype=np.float32,
        )
