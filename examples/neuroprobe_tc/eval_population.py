...
# ---- Load electrode 3D coordinates: X, Y, Z ----
root_dir = os.getenv("ROOT_DIR_BRAINTREEBANK", "neuroprobe/data/BrainTreebank")
...
for eval_name in eval_names:
    ...
    # Load neural data if it hasn't been loaded yet; NOTE: this is done here to avoid
    # unnecessary loading of neural data if the file is going to be skipped.
    if not neural_data_loaded:
        start_time = time.time()
        subject.load_neural_data(trial_id)
        subject_load_time = time.time() - start_time
        if verbose: log(f"Subject loaded in {subject_load_time:.2f} seconds", priority=0)
    ...
    # --------- Build electrode_xyz (X, Y, Z) for Transformer ---------
    electrode_xyz_for_transformer = None
    if classifier_type == 'transformer':
        if splits_type == "CrossSubject":
            raise NotImplementedError(
                "Transformer + CrossSubject + BrainPositionalEncodingXYZTime is not implemented, "
                "because ROI-level coordinates would need to be defined after region averaging."
            )

        # Map BrainTreebank subject id (e.g. 'btbank3') to coords CSV id (e.g. 'sub_3')
        train_sub_key = subject_identifier_to_coords_key(train_subject.subject_identifier)
        coords_sub = coords_df[coords_df['Subject'] == train_sub_key].set_index('Electrode')

        xyz_list = []
        for lab in train_subject.electrode_labels:
            if lab in coords_sub.index:
                row = coords_sub.loc[lab]
                xyz_list.append([row['X'], row['Y'], row['Z']])
            else:
                log(f"[Warning] No XYZ coords for electrode {lab} in subject {train_sub_key}", priority=1)

        if len(xyz_list) == 0:
            raise ValueError(
                f"No electrode XYZ coords found for subject {train_sub_key}, "
                "cannot run Transformer with BrainPositionalEncodingXYZTime."
            )

        # Final array of shape (n_electrodes, 3)
        electrode_xyz_for_transformer = np.array(xyz_list, dtype=np.float32)
    ...
    for bin_start, bin_end in zip(bin_starts, bin_ends):
        ...
        for fold_idx, fold in enumerate(folds):
            ...
            log("Standardizing data...", priority=2, indent=1)
            ...
            log("Training model...", priority=2, indent=1)
            ...
            if classifier_type == 'linear':
                ...
                X_train = X_train.reshape(original_X_train_shape)
                X_test = X_test.reshape(original_X_test_shape)
                clf = TransformerClassifier(random_state=seed, electrode_xyz=electrode_xyz_for_transformer)
            ...
            del X_train, y_train, X_test, y_test, train_probs, test_probs, y_test_filtered, test_probs_filtered, y_test_onehot, y_train_onehot, clf, scaler
